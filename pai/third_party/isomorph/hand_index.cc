...
INITIALIZER(hand_index_ctor) {
  ...
  suit_permutations = (uint_fast32_t(*)[SUITS])calloc(
      num_permutations, SUITS * sizeof(uint_fast32_t));
...
void count_configurations(uint_fast32_t round, uint_fast32_t configuration[],
                          void* data) {
  uint_fast32_t* counts = (uint_fast32_t*)data;
  ++counts[round];
}
...
out:;
  ...
  for (uint_fast32_t i = 0; i < SUITS;) {
    ...
    for (uint_fast32_t k = i; k < j; ++k) {
      indexer->configuration_to_suit_size[round][id][k] = (uint_fast32_t)size;
    }
...
void tabulate_permutations(uint_fast32_t round, uint_fast32_t count[],
  ...
  for (uint_fast32_t i = 0; i < SUITS; ++i) {
    ...
    uint_fast32_t smaller = std::popcount((this_bit - 1) & pi_used);
  ...
  while (low < high) {
    ...
    for (uint_fast32_t i = 0; i < SUITS; ++i) {
      uint_fast32_t thiss = count[pi[i]];
...
bool hand_indexer_init(uint_fast32_t rounds, const uint8_t cards_per_round[],
  ...
  for (uint_fast32_t i = 0; i < rounds; ++i) {
    indexer->configuration_to_equal[i] = (uint_fast32_t*)calloc(
        indexer->configurations[i], sizeof(uint_fast32_t));
    indexer->configuration_to_offset[i] =
        (hand_index_t*)calloc(indexer->configurations[i], sizeof(hand_index_t));
    indexer->configuration[i] = (uint_fast32_t(*)[SUITS])calloc(
        indexer->configurations[i], SUITS * sizeof(uint_fast32_t));
    indexer->configuration_to_suit_size[i] = (uint_fast32_t(*)[SUITS])calloc(
        indexer->configurations[i], SUITS * sizeof(uint_fast32_t));
  ...
  for (uint_fast32_t i = 0; i < rounds; ++i) {
    indexer->permutation_to_configuration[i] =
        (uint_fast32_t*)calloc(indexer->permutations[i], sizeof(uint_fast32_t));
...
hand_index_t hand_index_next_round(const hand_indexer_t* indexer,
  ...
  for (uint_fast32_t i = 0; i < indexer->cards_per_round[round]; ++i) {
    ...
    ranks[suit] |= rank_bit;
    shifted_ranks[suit] |=
        rank_bit >> std::popcount((rank_bit - 1) & state->used_ranks[suit]);
  }

  for (uint_fast32_t i = 0; i < SUITS; ++i) {
    assert(!(state->used_ranks[i] & ranks[i])); /* no duplicate cards */

    uint_fast32_t used_size = std::popcount(state->used_ranks[i]),
                  this_size = std::popcount(ranks[i]);
    state->suit_index[i] +=
  ...
  for (uint_fast32_t i = 0, remaining = indexer->cards_per_round[round];
       i < SUITS - 1; ++i) {
    uint_fast32_t this_size = std::popcount(ranks[i]);
