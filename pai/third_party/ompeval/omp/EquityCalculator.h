...
// Calculates all-in equities in Texas Holdem for given player hand ranges,
// board cards and dead cards. Supports both exact enumeration and monte carlo
// simulation.
class EquityCalculator {
 public:
  struct Results {
    ...
    // How many showdowns were actually evaluated (instead of using lookups or
    // isomorphism).
  ...
  // Start a new calculation. Returns false if calculation is impossible for
  // given hand ranges and board/dead cards. After calling start() succesfully,
  // wait() must be called in order wait for threads to finish. handRanges: hand
  // ranges for each player boardCards/deadCards: bitmasks for board and dead
  // cards enumerateAll: true for exact enumeration, false for monte carlo
  // stdevTarget: stops monte carlo when standard deviation is smaller than
  // this, use 0 for infinite simulation callback: function that is called
  // periodically with incomplete results updateInterval: how often callback is
  // called threadCount: number of threads to spawn, 0 for maximum parallelism
  ...
  // Wait for calculation to finish. Must always be called once for every
 ...
 private:
  ...
  struct BatchResults {
    BatchResults(unsigned nplayers) {
      for (unsigned i = 0; i < nplayers; ++i) playerIds[i] = i;
    }
  ...
  bool randomizeHoleCards(uint64_t& usedCardsMask, unsigned* comboIndexes,
                          Hand* playerHands, Rng& rng,
                          FastUniformIntDistribution<unsigned, 21>* comboDists);
  OMP_FORCE_INLINE void randomizeBoard(
      Hand& board, unsigned remainingCards, uint64_t usedCardsMask, Rng& rng,
      FastUniformIntDistribution<unsigned, 16>& cardDist);
  template <bool tFlushPossible = true>
  OMP_FORCE_INLINE void evaluateHands(const Hand* playerHands,
                                      unsigned nplayers, const Hand& board,
                                      BatchResults* stats, unsigned weight);
  void enumerate();
  void enumerateBoard(const HandWithPlayerIdx* playerHands, unsigned nplayers,
                      const Hand& board, uint64_t usedCardsMask,
                      BatchResults* stats);
  void enumerateBoardRec(const Hand* playerHands, unsigned nplayers,
                         BatchResults* stats, const Hand& board, unsigned* deck,
                         unsigned ndeck, unsigned* suitCounts, unsigned k,
                         unsigned start, unsigned weight);
  ...
  static unsigned transformSuits(HandWithPlayerIdx* playerHands,
                                 unsigned nplayers, uint64_t* boardCards,
                                 uint64_t* usedCards);
  static uint64_t calculateUniquePreflopId(const HandWithPlayerIdx* playerHands,
                                           unsigned nplayers);
  static Hand getBoardFromBitmask(uint64_t board);
  static std::vector<std::vector<std::array<uint8_t, 2>>> removeInvalidCombos(
  ...
  std::vector<CardRange>
      mOriginalHandRanges;  // Original ranges without before card removal.
  std::vector<std::vector<std::array<uint8_t, 2>>>
      mHandRanges;  // Ranges after card removal.
  ...
  // Precalculated results for 2 player preflop situations. Uses a sorted array
  // for lowest memory use.
